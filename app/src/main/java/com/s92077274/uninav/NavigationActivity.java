...
public class NavigationActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private List<LatLng> simulatedRoutePath;
    ...
    private void retrieveIntentData() {
        ...
        tvNextInstruction.setText("Navigating to: " + destinationLocation.name);
    ...
    public void onMapReady(@NonNull GoogleMap map) {
        ...
        if (isLiveLocationStart) {
            ...
            userMarker = googleMap.addMarker(new MarkerOptions()
                    .position(simulatedRoutePath.get(0))
                    .title("Your Location")
    ...
    private void addMarkersToMap() {
        ...
        if (!isLiveLocationStart) {
            LatLng startLatLng = new LatLng(startLocation.x, startLocation.y);
        ...
        LatLng destinationLatLng = new LatLng(destinationLocation.x, destinationLocation.y);
        googleMap.addMarker(new MarkerOptions()
    ...
    private void createSimulatedRoute() {
        simulatedRoutePath = new ArrayList<>();
        LatLng startLatLng = new LatLng(startLocation.x, startLocation.y);
        LatLng destinationLatLng = new LatLng(destinationLocation.x, destinationLocation.y);
        ...
        for (int i = 0; i <= numPoints; i++) {
            ...
            double lat = startLatLng.latitude + (destinationLatLng.latitude - startLatLng.latitude) * fraction;
            double lng = startLatLng.longitude + (destinationLatLng.longitude - startLatLng.longitude) * fraction;
            simulatedRoutePath.add(new LatLng(lat, lng));
        }
    }

    private void drawRoute() {
        if (googleMap == null || simulatedRoutePath == null || simulatedRoutePath.isEmpty()) return;

        PolylineOptions polylineOptions = new PolylineOptions()
                .addAll(simulatedRoutePath)
                .width(10)
                .color(Color.BLUE)
                .geodesic(true);
    ...
    private void zoomToFitRoute() {
        ...
        for (LatLng point : simulatedRoutePath) {
    ...
    private void simulateNavigation() {
        if (simulatedRoutePath == null || simulatedRoutePath.isEmpty()) return;

        runnable = new Runnable() {
            @Override
            public void run() {
                if (currentPathIndex < simulatedRoutePath.size()) {
                    LatLng currentSimulatedLatLng = simulatedRoutePath.get(currentPathIndex);
                    if (userMarker == null) {

                        userMarker = googleMap.addMarker(new MarkerOptions()
                                .position(currentSimulatedLatLng)
                                .title("Your Location")
                                .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE)));
                    } else {
                        userMarker.setPosition(currentSimulatedLatLng);
                    }


                    googleMap.animateCamera(CameraUpdateFactory.newLatLng(currentSimulatedLatLng), 1000, null);


                    updateInstruction(currentSimulatedLatLng);

                    currentPathIndex++;
                    handler.postDelayed(this, 2000);
                } else {
                    Toast.makeText(NavigationActivity.this, "You have arrived at your destination!", Toast.LENGTH_LONG).show();
                    tvNextInstruction.setText("Arrived at " + destinationLocation.name);
                    tvRemainingDistance.setText("0 km");
    ...
    private void updateInstruction(LatLng currentLatLng) {
        if (currentPathIndex == 0) {
            tvNextInstruction.setText("Starting navigation...");
        } else if (currentPathIndex < simulatedRoutePath.size() / 2) {
            tvNextInstruction.setText("Continue straight...");
        } else if (currentPathIndex == simulatedRoutePath.size() - 1) {
            tvNextInstruction.setText("You have arrived!");
        } else {
            tvNextInstruction.setText("Follow the blue line...");
        }
        ...
        tvRemainingDistance.setText(String.format(Locale.getDefault(), "Remaining: %.2f km", distanceKm));
    }
            ...
            public void onLocationResult(@NonNull LocationResult locationResult) {
                ...
                for (Location location : locationResult.getLocations()) {
                    if (location != null) {
                        ...
                        googleMap.animateCamera(CameraUpdateFactory.newLatLng(newLatLng), 1000, null);
                        updateInstruction(newLatLng);
                    }
                }
            }
        };
    ...
    protected void onResume() {
        ...
        if (isLiveLocationStart && ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION)
                == PackageManager.PERMISSION_GRANTED) {
            startLocationUpdates();
        } else if (!isLiveLocationStart && simulatedRoutePath != null && currentPathIndex < simulatedRoutePath.size()) {
    ...
    private void initSavedLocations() {
        ...
        savedLocationMap.put("Department of Mathematics and Computer Science", new MapPoint("Department of Mathematics and Computer Science", "Computer Science Building", 6.884542050021143f, 79.88382032910988f, "Academic")); // Corrected Longitude
